...
def find_teacher(class_to_teacher, x, y, Tl):
    ...
    priority_list = class_to_teacher[y][-1][0]

    while class_to_teacher[y][-1][1] < len(priority_list):

        ptr = class_to_teacher[y][-1][1]
        i = priority_list[ptr]

        class_to_teacher[y][-1][1] += 1
        '''
        if class_to_teacher[y][i] > 0 and Tl[x][i]["available"]:
            ...
            return i
            '''
        if class_to_teacher[y][i] > 0:
            class_to_teacher[y][i] -= 1
            if i < 17:  # real teacher
                Tl[x][i]["available"] = False
            return i
